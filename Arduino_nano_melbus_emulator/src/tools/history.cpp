 ...
 * @return MELBUS_byte_t Byte at the given location
 */
MELBUS_byte_t ByteHistory::GetByteAt(uint32_t location)
{
 ...
 */
bool ByteHistory::Append(const MELBUS_byte_t byte)
{
    for(int32_t i = ByteHistory::GetSize() - 2; i >= 0; i--)
    {
        MELBUS_byte_t temp = m_history[i];
        m_history[i+1] = temp;
    }
 ...
 * @param clearValue 
 */
void ByteHistory::Clear(const MELBUS_byte_t clearValue)
{

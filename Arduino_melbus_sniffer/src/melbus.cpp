...
static uint32_t f_timingBegin;
static uint32_t f_timingEnd;
static uint8_t f_timingCounter;

static uint32_t f_times[TIME_SIZE];
static uint8_t f_timesIndex;
...
bool MELBUS_Init()
{
    ...
    memset(f_times, 0, sizeof(f_times));

    pinMode(MELBUS_CLK_PIN, INPUT_PULLUP);
    pinMode(MELBUS_BUSY_PIN, INPUT_PULLUP);
    pinMode(MELBUS_DATA_PIN, INPUT_PULLUP);

    attachInterrupt(digitalPinToInterrupt(MELBUS_CLK_PIN), MelbusRxInterrupt, FALLING);
    return true;
}
...
bool MELBUS_IsBusy()
{
    return (digitalRead(MELBUS_BUSY_PIN) == LOW);
}
